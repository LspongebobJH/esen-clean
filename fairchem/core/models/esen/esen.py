...
from fairchem.core.common.utils import (cg_change_mat, conditional_grad,
                                        irreps_sum)
from fairchem.core.models.base import GraphModelMixin, HeadInterface

from .common.rotation import init_edge_rot_mat, rotation_to_wigner
...
from .nn.layer_norm import (EquivariantLayerNormArray,
...
class eSEN_Backbone(nn.Module, GraphModelMixin):
    def __init__(
        ...
        condition_frac_diff: bool = False,
        n_frequencies: int = 10,
        condition_cosine: bool = False,
    ):
        super().__init__()
        ...
        # NOTE: add fractional distance into edge embedding
        self.condition_frac_diff = condition_frac_diff
        if condition_frac_diff:
            self.frac_distance_embedding = SinusoidsEmbedding(n_frequencies=n_frequencies, period=1)
            frac_distance_dim = n_frequencies * 3 * 2
        else:
            frac_distance_dim = 0

        self.consider_cosine = condition_cosine
        if condition_cosine:
            cosine_dim = 3
        else:
            cosine_dim = 0
        ...
        self.edge_channels_list = [
            self.num_distance_basis + 2 * self.edge_channels + frac_distance_dim + cosine_dim,
            self.edge_channels,
            self.edge_channels,
        ]
    ...
    def forward(self, data_dict) -> dict[str, torch.Tensor]:
        ...
        if self.condition_frac_diff:
            frac = cart_to_frac(data_dict['cell'], data_dict['pos'], data_dict['natoms'])
            edge_frac_diff = (frac[graph_dict["edge_index"][1]] - frac[graph_dict["edge_index"][0]]) % 1.0
            edge_frac_distance_embedding = self.frac_distance_embedding(edge_frac_diff)
            x_edge = torch.cat((x_edge, edge_frac_distance_embedding), dim=1)

        if self.consider_cosine:
            # Compute cosine similarity between edge vectors and cell vectors
            edge_vectors = graph_dict["edge_distance_vec"]
            batch_edge = graph_dict["batch_full"][graph_dict["edge_index"][0]]
            cosines = torch.cosine_similarity(
                edge_vectors[:, None],
                data_dict["cell"][batch_edge],
                dim=-1,
            )
            x_edge = torch.cat((x_edge, cosines), dim=1)
...
def cart_to_frac(cell, cart, num_atoms):
    cart = cart.double()
    cell = cell.double()

    pbc_nodes = torch.repeat_interleave(cell, num_atoms, dim=0)

    # for cells that masked as [1e-3] * 9 (t=0), we also consider them as null_pbc 
    # null_pbc = pbc_nodes.abs().sum(dim=[1, 2]) == 0
    null_pbc = pbc_nodes.abs().sum(dim=[1, 2]) < 1e-2   
    ...
    frac = torch.linalg.solve(pbc_nodes_masked.transpose(1, 2), cart)
    return frac.float()