...
from fairchem.core.datasets.target_metadata_guesser import \
...
class AseDBDataset(AseAtomsDataset):
    ...
    def _load_dataset_get_ids(self, config: dict) -> list[int]:
        if isinstance(config["src"], (list, ListConfig)):
            filepaths = []
            for path in sorted(config["src"]):
                if os.path.isdir(path): # directory
                    _files = sorted(glob(f"{path}/*"))
                    if distutils.is_master():
                        print(f"Reading {path}...\nfound {len(_files)} files")
                    filepaths.extend(_files)
                elif os.path.isfile(path): # file
                    filepaths.append(path)
                else: # file pattern
                    _files = sorted(glob(f"{path}"))
                    if len(_files) == 0: # no files found
            ...
            if distutils.is_master():
                print(f"Found by glob: {len(filepaths)} files")
        else:
            filepaths = sorted(glob(config["src"]))
            if distutils.is_master():
                print(f"Found by glob: {len(filepaths)} files")

        # NOTE: yuanhangtangle,2025-07-31 
        # if a file pattern like "**/*.aselmdb" is used, the filepaths will be a list of file paths
        # we need to load the metadata from the filepaths
        self.paths = [Path(path) for path in filepaths]
        # if distutils.is_master():
        #     print(f"{len(self.paths)} paths")
        # end of NOTE

        self.dbs = []

        # for path in tqdm(filepaths, desc="Loading ASE databases"):
        if distutils.is_master():
            print(f"Loading {len(filepaths)} ASE databases")
        _n = len(filepaths) // 10 if len(filepaths) > 10 else 1
        for i, path in enumerate(filepaths):
            if i % _n == 0 and distutils.is_master():
                print(f"Loading {i} / {len(filepaths)} ASE databases")
        ...
        if distutils.is_master():
            print(f"Total number of samples: {sum(idlens)}")
